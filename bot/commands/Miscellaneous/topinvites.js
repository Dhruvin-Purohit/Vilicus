...
module.exports = class TopInvites extends Command {
    ...
    async exec(message) {
        ...
        if(!invites || invites.length < 1) return message.channel.send(`There are no invites..`)
        const good = invites.filter(i => i.uses > 0).sort((a, b) => b.uses - a.uses).first(10)
        if(!good || good.length < 1) return message.channel.send(`None of the invites have been used..`)
