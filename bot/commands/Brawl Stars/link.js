...
module.exports = class extends Command {
    ...
    async exec(message, { tag }) {
        if(message.guild.db.gdb.bsclub) return message.channel.send(`One Club is already linked to this server, unlink that and try again`)
        if(!tag) return message.channel.send(`No Club tag provided`)
        let tagrgx = /^#?[0-9A-Z]/i
        if(!tagrgx.test(tag)) return message.channel.send(`Invalid Club tag provided`)
        ...
        try {//if that does not work i will change the clubs club //I hate that i don't have internet
            ...
            if (!stats.reason) {
                ...
                return message.channel.send(`${message.author}, The Brawl Stars club,\n${dmd.bold `${stats.name}`}(${stats.tag}) has been linked to this discord server successfully`)

            } else if (res.status === 404) {
                return message.channel.send(`Next time when you run the command, give me an actually valid Brawl Stars club tag!`)
            } else {
